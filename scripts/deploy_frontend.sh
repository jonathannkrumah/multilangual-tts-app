...
APP_CLIENT_ID=$(terraform -chdir="$INFRA_DIR" output -raw cognito_app_client_id)
REGION=$(terraform -chdir="$INFRA_DIR" output -raw region 2>/dev/null || echo "us-east-1")
...
if [[ -z "$USER_POOL_ID" ]]; then
  echo "Failed to resolve Cognito User Pool ID from outputs" >&2
  exit 1
fi

if [[ -z "$APP_CLIENT_ID" ]]; then
  echo "Failed to resolve Cognito App Client ID from outputs" >&2
  exit 1
fi
...
echo "  - Users must sign up/sign in to use the app"
